...
public:
  ...
  map<char, string> BuildEncodingTable(shared_ptr<freq_info> root);
    ...
    // BuildEncodingTableHelper is a recursive function that traverses the Huffman tree
    // and generates a lookup table mapping each symbol to its corresponding binary encoding.
    void BuildEncodingTableHelper(shared_ptr<freq_info> node, map<char, string>& encoding_table, string& code);
