...
void mainMenu();
void readWord(const int, ifstream &, vector<string> &);
int letters(const vector<string> &, vector<char> &, const int);
int hangmanFigure(int, const vector<string> &, int);
int guessLetters(const vector<string> &, vector<char> &, int &);
...
int guessLetters(const vector<string>& words, vector<char>& blank_letter, int& word_index){
    ...
    while (i < MAX_GUESS) {
        ...
        // Calculate the rounded score as a percentage of letters found out of the total
